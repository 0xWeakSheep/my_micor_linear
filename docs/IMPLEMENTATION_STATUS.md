...
更新时间：2026-07-17。本文描述当前仓库中已经实现的功能与交付边界，适用于单实例、100 人以内的自托管团队。
...
- 管理与集成：工作区/团队/成员设置、API Key 与作用域、REST API v1、签名 Webhook、投递历史与手动重放、CSV 导入导出、审计日志、Activity、Outbox 和 SSE 失效同步。
...
- 后台任务默认由应用内单实例调度器执行，也可关闭后改用带互斥锁的外部 cron；Webhook worker 使用租约 token 阻止失租进程覆盖接管结果。
