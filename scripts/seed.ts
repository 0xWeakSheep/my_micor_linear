...
async function seed(): Promise<void> {
  ...
  try {
    ...
    insertRows(database, "webhooks", [
      { id: "webhook_local", workspace_id: WORKSPACE_ID, name: "Local release bot", url: "http://localhost:4000/hooks/micro-linear", secret_hash: hashOpaqueToken("micro-linear-seed-webhook-secret"), signing_secret_encrypted: sealWebhookSecret("micro-linear-seed-webhook-secret"), events_json: JSON.stringify(["issue.created", "issue.updated", "project-update.created"]), is_active: 0, created_by_id: "usr_demo", created_at: CREATED, updated_at: NOW },
    ]);
